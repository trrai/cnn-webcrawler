            ...
            if (isRespondingWebsite(url) && statusCheck())
            {
                ...
                using (var client = new WebClient())
                {
                    try
                    {
                        ...
                        try
                        {
                            ...
                            if (!url.Contains("bleacherreport") && !url.Contains("money.cnn.com"))
                            {
                                textNodes = web.DocumentNode.SelectNodes("//*[contains(@class, 'zn-body__paragraph')]");
                            }
                            else { 
                                textNodes = web.DocumentNode.SelectNodes("//p");
                            }
                        ...
                        {
                            bodyText = "Could not parse body text";
                            System.Diagnostics.Debug.WriteLine("Error Parsing Body Text: " + e.Message);
                        }

                        //Capture publishing dates
                ...
                foreach (var word in keywords)
                {
                    if (Regex.IsMatch(word, @"^[a-zA-Z]+$"))
                    {
                        ...
                        try {
                        TableOperation insertOperation = TableOperation.InsertOrReplace(newWebsite);
                        DBManager.getResultsTable().Execute(insertOperation);
                        }
                        catch
                        {
                            System.Diagnostics.Debug.WriteLine("Unable to Insert: " + word + " | " + url);
                        }
                    }
                    else
                    {
                ...
                //get current count
                int updatedCount = keywords.Count();
                ...
                if (retrievedResult.Result != null)
                {
                    ...
                    updatedCount = (int)oldCount + keywords.Count();
